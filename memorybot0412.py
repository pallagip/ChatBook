...
for user_id in st.session_state.users:
    for i in range(len(st.session_state['generated'][user_id])):
        combined_conversations.append((user_id, i, "question"))
        combined_conversations.append((user_id, i, "answer"))

# Sort the combined list in reverse order based on the index
sorted_conversations = sorted(combined_conversations, key=lambda x: x[1], reverse=True)

# Display the conversation history using the sorted list
with st.expander("Combined Conversation", expanded=True):
    for user_id, i, msg_type in sorted_conversations:
